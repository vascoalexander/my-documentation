---
sidebar_label: "References"
title: "References"
sidebar_position: 1
toc_max_heading_level: 4
description: ""
---
## **General**
- [**Unicode**](https://home.unicode.org/)  
- [**DB-Engines**](https://db-engines.com/de/)   
- [**Bash Reference Manual**](https://www.gnu.org/savannah-checkouts/gnu/bash/manual/bash.html)  
- [**Linux Manpages Online**](http://man.he.net/)  

## **Python**
- [**Python Documentation**](https://docs.python.org/3/index.html)  
- [**PEP 8 - Style Guide for Python Code**](https://peps.python.org/pep-0008/)  
- [**Python Module Index**](https://docs.python.org/3/py-modindex.html)  
- [**Python Package Index**](https://pypi.org/)  
<br />

- [**W3Schools - Python Ref**](https://www.w3schools.com/python/python_reference.asp)  
- [**Python String Formating**](https://pyformat.info/)

### Modules
- [**tkinter Docs**](https://docs.python.org/3/library/tkinter.ttk.html#)  
- [**ttkbootstrap**](https://ttkbootstrap.readthedocs.io/en/latest/)  
- [**ttkbootstrap docs**](https://ttkbootstrap.readthedocs.io/en/version-0.5/index.html)  
- [**tkinter event binding**](https://www.pythontutorial.net/tkinter/tkinter-event-binding/)  
- [**CustomTkinter Docs**](https://customtkinter.tomschimansky.com/documentation/)  
- [**PyInstaller Manual**](https://pyinstaller.org/en/stable/)

### Frameworks
- [**Django Docs**](https://docs.djangoproject.com/en/5.1/)
- [**Flask User Guide**](https://flask.palletsprojects.com/en/stable/#)

## **C#**

- [**C# Language Reference**](https://learn.microsoft.com/en-us/dotnet/csharp/language-reference/)  
- [**C# Standard Specifications**](https://learn.microsoft.com/en-us/dotnet/csharp/specification/overview)

## **Web-Development**

### General
- [**W3C**](https://www.w3.org/)
- [**W3Schools**](https://www.w3schools.com/)
- [**cssreference.io**](https://cssreference.io/)
- [**htmlreference.io**](https://htmlreference.io/)

### Frameworks
- [**Bootstrap**](https://getbootstrap.com/docs/5.3/getting-started/introduction/)
- [**React**](https://react.dev/reference/react)
- [**tailwindcss**](https://tailwindcss.com/docs/installation/using-vite)
- [**Angular**](https://angular.dev/overview)
- [**Vue.js**](https://vuejs.org/guide/introduction.html)

### Libraries
- [**Shoelace Web Components**](https://shoelace.style/)  
- [**jQuery**](https://api.jquery.com/)

### Resources
- [**Font Awesome 4**](https://fontawesome.com/v4/icons/)

## **Development Tools**
- [**Jira**](https://www.atlassian.com/software/jira/guides/getting-started/introduction#what-is-jira-software)  
- [**Jenkins**](https://www.jenkins.io/doc/book/getting-started/)  
- [**Docker**](https://docs.docker.com/)  
- [**Kubernetes**](https://kubernetes.io/docs/home/)  
- [**git**](https://git-scm.com/doc)  
- [**gulp.js**](https://gulpjs.com/docs/en/getting-started/quick-start)  
- [**grunt.js**](https://gruntjs.com/)
- [**Vite**](https://vite.dev/guide/)
- [**Yarn**](https://yarnpkg.com/cli)
- [**NPM**](https://docs.npmjs.com/)



